...
class PlayCommand extends BaseSlashCommand {
    ...
    async run(client, message) {
        ...
        client.reply(message, 'Поиск...', 'Производится поиск по запросу: **' + query + '**');
        ...
        const embed = new MessageEmbed()
            .setTitle(client.customEmojis.yes + ' Успешно!')
            .setDescription((result.type === 'PLAYLIST' ? `Добавлено **${result.tracks.length}** треков из плейлиста **${result.playlistName}**` : `Трек **${result.tracks[0].title}** добавлен в очередь`).toString())
            .setColor(client.colors.success);
        return message.editReply({ embeds: [embed] }).catch(console.error);
    }
}
