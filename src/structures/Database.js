...
module.exports = class extends MongoClient {
    ...
    async connect() {
        await logger.send('Database', 'Establishing a connection to the cluster...');
        await this.client.connect();
        await logger.send('Database', 'Successfully established connection to the cluster')
    }
    ...
    pullOne(collectionName, findParameters, data) {
        try {
            return this.client.db('Main').collection(collectionName).updateOne(findParameters, { $pull: data });
        } catch(error) {
            void logger.error('Database', 'An error occurred when executing the pushOne action\n' + error);
        }
    }
