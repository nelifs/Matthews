...
${client.functions.progressBar(track, player, 17)}
    ...
    const messsage = await client.channels.cache.get(player.textId)?.send({ embeds: [embed], components: [row, row1] })
    const messsage1 = await client.channels.cache.get(player.voiceId)?.send({ embeds: [embed], components: [row, row1] })
    ...
    const interval = setInterval(() => {
        ...
        messsage1.edit({ embeds: [embed.setDescription(`
Автор: **${track.author}**
Повтор: **${loop}**
Состояние плеера: **${!player.paused ? "Проигрывается трек" : "На паузе"}**
Громкость: **${~~(player.volume * 100).toFixed(0)}/${client.lavalink.maxVolume}**
...
${client.functions.progressBar(track, player, 17)}
        ...
        messsage.edit({ embeds: [embed.setDescription(`
Автор: **${track.author}**
Повтор: **${loop}**
Состояние плеера: **${!player.paused ? "Проигрывается трек" : "На паузе"}**
Громкость: **${~~(player.volume * 100).toFixed(0)}/${client.lavalink.maxVolume}**
